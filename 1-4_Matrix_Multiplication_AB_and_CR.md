...
- **의미**: $C$ 의 첫 번째 줄은 $B$ 의 줄들을 $A$ 의 첫 번째 줄 성분만큼 섞은 결과입니다.

$$
1 \begin{bmatrix} 5 & 6 \end{bmatrix} + 2 \begin{bmatrix} 7 & 8 \end{bmatrix} = \begin{bmatrix} 19 & 22 \end{bmatrix} \quad (\text{C의 1번 줄})
$$

### 1.4 블록 방식 (Outer Product): "조각 조립"
$A$ 의 열과 $B$ 의 행을 곱해 만들어지는 얇은 판데기(Rank-1 행렬) 들을 모두 더합니다.
...
- 1번 기둥: $[1, 2, 0]^{T}$ (독립)
- 2번 기둥: $[0, 1, 1]^{T}$ (독립)
- 3번 기둥: $[2, 4, 0]^{T}$ (1번 기둥의 2배 $\rightarrow$ **중속**, 버림!)
